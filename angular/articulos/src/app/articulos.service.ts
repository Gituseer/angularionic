...
export class ArticulosService {
  ...
  constructor(private http:HttpClient) {
      ...
      this.encabezados={headers: new HttpHeaders({
      	  ...
      	  'Autorization': 'Bearer'+localStorage.getItem('sessionToken')
   ...
   traerArticulos():Observable<any>{
    ...
    crearArticulo(articulo:any):Observable<any>{
    	return this.http.post<any>(this.url,articulo);
    }

    editarArticulo(articulo:any):Observable<any>{
    	return this.http.put<any>(this.url+'/'+articulos.id, articulo,this.encabezados);
    }
